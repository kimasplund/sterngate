...
| D4 | One `MapProvenance` enum (`Unverified` default, `Synthetic`, `Fallback`, `Scanned`) lives in `calibrator/map.rs`, is a required field on `EcuMap` and an integrity-covered optional field on `PatchFlashMap`/`DtcMask`. In Phase 0 nothing is stamped `Scanned`, not even the ROM-scanned SVBL. | Phase 2 defines what "scanned" means; until then every generated flash patch is non-executable. |
