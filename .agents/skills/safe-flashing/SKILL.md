...
3. **One-Click Staging**:
   ...
   - `sterngate mod apply` and `POST /api/v1/mods/apply` follow the same voltage resolution rule: the CLI reads the OpenPort Pin-16 ADC through `VehicleInterface::measure_battery_voltage` and refuses on adapters without a sensor; the REST route requires `battery_voltage` and the connected VIN.
