...
| `sterngate_verify_flash_staging` | `target_module`, `expected_hw_id` (both required); `expected_sw_id`, `sha256`, `crc32` (opt — default to the simulated 4096-byte ROM's own checksums, `expected_sw_id` defaults to `1037372332`) | Runs the real pre-flight (voltage, SHA-256/CRC32, F192 identity, flash_length) against the built-in virtual ECU and returns its report — `passed`, `hw_id_match`, `checksum_match`, `details`, the `manifest` compared, `simulated: true`. |
...
| `sterngate_flash_ecu` | `target_module` (opt), `battery_voltage` (opt), `dry_run` (opt) | Safely executes or simulates detached ECU flashing with battery voltage interlock ($\ge 12.5\text{ V}$). `dry_run: true` runs the same real pre-flight as `sterngate_verify_flash_staging` against the virtual ECU (no hardcoded pass) and returns `preflight_passed`, `hw_id_match`, `checksum_match`, `details`, `manifest`, `simulated: true`. |
...
| `sterngate_apply_community_mod` | `mod_content`, `vin`, `battery_voltage` (all required) | **Simulated.** Runs the package against the built-in virtual ECU, never a real vehicle: the response carries `"simulated": true` and a `[SIMULATED against the virtual ECU]` message prefix. Provenance, integrity, voltage floor and live byte preconditions are all enforced against that virtual ECU. No `force` argument exists; sending one is an error. Use the CLI (`sterngate mod apply`) or the REST API to touch real hardware. |
